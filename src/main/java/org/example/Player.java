...
public  class Player {
    ...
    private Set<String> characterTypes = new HashSet<>();
...
//  Verify that Character type is unique for the player
    private boolean isCharactersTypeUnique() {
        for (Character character : this.characters) {
            this.characterTypes.add(character.getType().toLowerCase());
        }
...
//    Allow to add a Character
    public void addCharacter(Character character) {
        if(!this.characterTypes.contains(character.getType().toLowerCase())) {
            throw new IllegalArgumentException("The type of characters must be unique");
        }
        ...
        characterTypes.add(character.getType().toLowerCase());
